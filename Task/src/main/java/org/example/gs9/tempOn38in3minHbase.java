...
import org.example.HbaseWriter;
import org.example.KafkaConnect;

import java.text.SimpleDateFormat;
import java.util.Date;

public class tempOn38in3minHbase {
...
//        environmentDataSingleOutputStreamOperator.print();
        ...
        warningTime.print();
        //转换格式以存入hbase
        SingleOutputStreamOperator<Tuple2<String, Tuple3<String, String, String>[]>> intoHbase = warningTime.map(new MapFunction<Tuple2<String, String>, Tuple2<String, Tuple3<String, String, String>[]>>() {
            @Override
            public Tuple2<String, Tuple3<String, String, String>[]> map(Tuple2<String, String> wangning) throws Exception {
                String date = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss:SSS").format(new Date());
                String rowkey = wangning.f0 + "-" + date;
                Tuple3<String, String, String>[] tuple3s = new Tuple3[2];
                tuple3s[0]=new Tuple3<>("info","machine_id",wangning.f0);
                tuple3s[1]=new Tuple3<>("info","total_produce",wangning.f1+"");
                return new Tuple2<>(rowkey, tuple3s);
            }
        });
        intoHbase.addSink(new HbaseWriter().hbaseWriter("gyflinkresult:EnvTemperatureMonitor"));
        env.execute("tempOn38in3minHbase");
    }
}
