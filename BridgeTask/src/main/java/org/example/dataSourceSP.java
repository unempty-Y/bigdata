...
//    private static final int PEAK_HOURS_END_EVENING = 19 * 60; // 晚高峰时间结束于下午7点
    ...
    private static final double DAMAGE_V1 = 0.1; // 初始损伤基数，自定义参数
    private static final double DAMAGE_V2 = 0.6; // 初始损伤变动范围，自定义参数 V1+V2*随机
            ...
            while (true) {
                ...
                for (Abutment abutment : abutments) {
                    ...
                    if (offset >= DAMAGE_THRESHOLD && v < VOIDRATE) {
                        ...
                        double damageCount = (1 - offset) * 0.8 + (1 - offset) * 0.2 * random.nextDouble();
                        double weight = 0;
                        for (Slab slab : slabs)
                    ...
                    for (Abutment abutment : abutments) {
                        if (abutment.bridgeid.equals(bridge.bridgeid) && abutment.pierno.equals("01")) {
                            if (random.nextDouble() < VehicleUp) { // 新车上桥概率
                                double weight = 100 + random.nextDouble() * 800; // 随机生成车辆的重量
                ...
                for (Vehicle vehicle : vehicles) {
                    for (Abutment abutment : abutments)
                        if (abutment.bridgeid.equals(vehicle.bridgeid) && abutment.abutmentno.equals(String.format("%02d", vehicle.abutmentno))) {
                            ...
                            double damage = (1 - offset) * 0.8 + (1 - offset) * 0.2 * random.nextDouble();
...
//                    }
                    ...
                    //填入
                    ForceTableSP forceTable;
                    double offset = abutment.damageCount;
                    double damage = (1 - offset) * 0.8 + (1 - offset) * 0.2 * random.nextDouble();
                    if (abutment.force<void_threshold)
                        forceTable = new ForceTableSP(abutmentno, bridgeid, pierno, forcevalue, time_stamp, random.nextFloat() * (float) damage * 50 + 10, random.nextFloat() * (float) damage * 70 + 10, 0,1);
                    else if (abutment.force>overload_threshold)
                        forceTable = new ForceTableSP(abutmentno, bridgeid, pierno, forcevalue, time_stamp, random.nextFloat() * (float) damage * 50 + 20, random.nextFloat() * (float) damage * 70 + 30, 1,0);
                    else
                        forceTable = new ForceTableSP(abutmentno, bridgeid, pierno, forcevalue, time_stamp, random.nextFloat() * (float) damage * 50 + 10, random.nextFloat() * (float) damage * 70 + 10, 0,0);
                    //改为json数据
                    Gson gson = new Gson();
                    ...
                    try (Connection conn1 = DriverManager.getConnection(URL, USER, PASSWORD)) {
                        insertForceTableSP(conn1, forceTable, "forcetablesp");//mysql输出反力表
...
//                    socket.send(packet);//端口输出
    ...
    public static void insertForceTableSP(Connection conn, ForceTableSP forceTableSP, String tbName) {
        String sql = "INSERT INTO " + tbName + " (abutmentno, bridgeid, pierno, forcevalue, time_stamp, temperature, humidness, overload,voidwarning) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
        try (PreparedStatement pstmt = conn.prepareStatement(sql)) {
            pstmt.setString(1, forceTableSP.getAbutmentno());
            pstmt.setString(2, forceTableSP.getBridgeid());
            pstmt.setString(3, forceTableSP.getPierno());
            pstmt.setFloat(4, forceTableSP.getForcevalue());
            pstmt.setString(5, forceTableSP.getTime_stamp());
            pstmt.setFloat(6, forceTableSP.getTemperature());
            pstmt.setFloat(7, forceTableSP.getHumidness());
            pstmt.setInt(8, forceTableSP.getOverload());
            pstmt.setInt(9, forceTableSP.getVoidwarning());
            pstmt.executeUpdate(); // 执行更新，不关心影响的行数
        } catch (SQLException e) {
            e.printStackTrace();
            try {
     ...
     * @input 反力表数据
     */
    public static void insertyujing(Connection conn, ForceTableSP forceTable, String tbName, int flag) {
    ...
    public static class ForceTableSP {
        ...
        private int overload; // 使用 int 类型对应于 TINYINT
        private int voidwarning; // 使用 int 类型对应于 TINYINT

        public ForceTableSP( String abutmentno, String bridgeid, String pierno, float forcevalue, String time_stamp, float temperature, float humidness, int overload, int voidwarning) {
            this.abutmentno = abutmentno;
            this.bridgeid = bridgeid;
            this.pierno = pierno;
        ...
        public int getVoidwarning() {
            return voidwarning;
        }

        public void setVoidwarning(int voidwarning) {
            this.voidwarning = voidwarning;
        }
    }
}
