...
//    private static final int PEAK_HOURS_END_EVENING = 19 * 60; // 晚高峰时间结束于下午7点
    ...
    private static final double DIRTY_RATE = 0.03; //脏数据占比
    private static final double DAMAGE_ACC = 0.0001; // 桥梁损伤积累速率，自定义参数
    private static final double DAMAGE_THRESHOLD = 0.8; // 脱空开始阈值，自定义参数
    private static final double DAMAGE_V1 = 0.165; // 初始损伤基数，自定义参数
    private static final double DAMAGE_V2 = 0.33; // 初始损伤变动范围，自定义参数 V1+V2*随机
                    ...
                    for (Abutment abutment : abutments) {
                        if (abutment.bridgeid.equals(bridge.bridgeid) && abutment.pierno.equals("01")) {
                            if (random.nextDouble() < VehicleUp) { // 新车上桥概率
                                double weight = 100 + random.nextDouble() * 500; // 随机生成车辆的重量
