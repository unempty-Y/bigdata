...
//  val refundOrder: OutputTag[(String, Double)] = new OutputTag[(String, Double)]("refundtag") {}
...
//      .filter(_._1=="totalprice")
      .print()
//      .addSink(new RedisSink[(String, Double)](config, new RedisSinkmapper))
    val refundcountminute = dataStream.filter(_._1.contains("totalrefundordercount"))
      .keyBy(_._1)
      .sum(1)
//refundcountminute.addSink(new RedisSink[(String, Double)](config, new RedisSinkmapper))
  ...
  class PaymentCountPF extends ProcessFunction[Order_master_data, (String, Double)] {
    lazy val PaymentCount: ValueState[Double] = getRuntimeContext.getState(new ValueStateDescriptor[Double]("PaymentCount", classOf[Double]))
    ...
    override def processElement(i: Order_master_data,
      ...
      if (tuple.contains(i.order_status)) {
        ...
        collector.collect("totalrefundordercount", i.payment_money)
    }
  }
