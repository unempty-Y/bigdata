...
data.print()
    val Hbase1: DataStream[String] = data
      .filter(line => line.contains("order_id"))
    Hbase1.addSink(new writeHbase(bigdata1, "test", "order_master", "info"))

    val Hbase2: DataStream[String] = data
      .filter(line => line.contains("order_detail"))
    Hbase2.addSink(new writeHbase(bigdata1, "test", "order_detail", "info"))
