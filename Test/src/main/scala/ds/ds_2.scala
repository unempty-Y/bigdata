...
object ds_2 {
  ...
  def main(args: Array[String]) {
    ...
    // 侧输出流处理
    val fefundOrderStream = orderStream.getSideOutput[(String, Double)](refundOrder)
    // 将侧流数据发往redis sink
    fefundOrderStream.addSink(new RedisSink[(String, Double)](config, new RedisSinkMapper))
    // execute program
  ...
  class MyOrderProcessFunction extends ProcessFunction[(Int, OrderEvent), (String, Double)] {
    ...
    // 累加最终退款金额
    private var lastRefundOrderAmount: ValueState[Double] = _

    // 初始化
    override def open(parameters: Configuration): Unit = {
      lastOrderAmount = getRuntimeContext.getState(new ValueStateDescriptor[Double]("lastAmount", classOf[Double]))
      lastRefundOrderAmount = getRuntimeContext.getState(new ValueStateDescriptor[Double]("lastRefundAmount", classOf[Double]))
    }
    ...
    override def processElement(input: (Int, OrderEvent),
      ...
      // 如果是“退回完成”状态的订单
      else if ("1006" == currentOrderStatus) {
        // 将当前订单记录的退款金额累加到上一次的退款总金额上
        var accRefundAmount = lastRefundOrderAmount.value() + input._2.final_total_amount

        // 将当前退款总金额发送到侧输出流"refund"
        context.output(refundOrder, ("totalrefundordercount", accRefundAmount))

        // 更新状态
        lastRefundOrderAmount.update(accRefundAmount)
      }
    }
  }
